...
export const ROUTES: readonly RouteEntry[] = [
  ...
  },
  {
    ...
    notes:
      ...
      'with the configured one, or a JWKS with no key — and will answer the same until somebody changes the configuration. That is the whole ' +
  ...
  },
  {
    ...
    audience: 'client', auth: 'none', rateLimited: false, ownerTier: false, status: 302,
    params: [], query: clientOidcCallbackQuery, request: null, response: null, errors: [], transport: 'http',
    notes:
      ...
      'it was given. ' +
      '\n\n**Not rate limited**, and unlike the provider listing beside it the reason is not that the work is cheap: the caller here is the ' +
      'identity provider redirecting somebody\'s browser, so a limiter would drop real sign-ins on the strength of traffic none of those ' +
      'people sent. The cost of this flow is metered at `start`, which is the door an app user actually opens. \n\nThe query is the ' +
      ...
      '\n\n**It lists no error codes because it emits none.** Every outcome, success and failure alike, is a `302` to the app\'s own ' +

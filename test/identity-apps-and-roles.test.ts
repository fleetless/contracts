...
describe('apps, keys and roles', () => {
  it('identifies an app by a slug, like a service', () => {
    ...
    expect(
      app.safeParse({ id: UUID, org_id: UUID2, name: 'Fleet Ops', identifier: 'fleet-ops', group_id: UUID2, robot_ids: [UUID], accepts_dynamic_clients: false, mcp_enabled: false, default_role_id: null, created_at: NOW })
        .success,
    ).toBe(true)
  })

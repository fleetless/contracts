 ...
 *
 * Written **with** the contracts delta, not after — see the note at the top
 * of `deletion-and-health.test.ts`: additions aren't self-policing and need
 * deliberate tests.
 *
 * Each test below names the *confusion* it removes: seven places where two
 * different things shared one address, and the platform silently picked one.
 */
...
describe('error codes', () => {
  it('registers every code this area introduced', () => {
    ...
    // way in this time, not by a reviewer on the way out.
  ...
  it('keeps a malformed id apart from a missing thing', () => {
    ...
    // client with a bad URL got a clean 404, and its developer went looking
    // for a robot they never lost.
...
describe('naming a job', () => {
  it('lets a cancel say WHICH job, and still lets it say "whatever is running"', () => {
    ...
    // client cancelling its own job means the first — and until now it
    // couldn't say so, so a cancel landing just after its job ended could
    // stop the next caller's job on the same slug.
...
describe('naming how long a caller will wait', () => {
  ...
  it('refuses a patience below the floor, because impatience reaches the robot', () => {
    // Omitting the field says nothing; a number too small to satisfy is
    // different — measured in review, `patience_ms: 1` on an action made the
    // bridge report `goal_timeout` then issue a CORRECTIVE CANCEL against a
    // goal the server accepted a moment later. An unreachable deadline
    // doesn't just error, it stops a machine. Repeatable.
...
describe('the same thing over both transports', () => {
  it('lets a REST cancel carry an id, and lets it carry nothing at all', () => {
    // The body is fully optional: `POST .../cancel` used to be bodyless and
    // every existing caller still sends nothing. One of this package's worst
    // bugs was a bodyless POST rejected outright — it took cancel, publish,
  ...
  it('lets a socket caller state a patience, because parity is a rule', () => {
    ...
    // first attempt gave `patience_ms` to the REST body only — but the SDK
    // invokes exclusively over realtime, so the field would have shipped
    // documented and unreachable for every SDK caller. Four such methods
    // have shipped that way before; this one was caught first.
  ...
  it('refuses a misspelled field instead of silently widening the request', () => {
    // Both shapes strip unknown keys, and here that fails UNSAFE: the id
    // disappears, leaving the slug-wide cancel or the identity-wide release —
    // the most destructive reading of a request nobody made. `{jobId: ...}`
    // answered 200 and stopped the job actually running; `?sessionid=`
    // released both holds and stranded the other tab.
...
describe('naming a publish attempt', () => {
  ...
  it('does NOT enforce the pairing rule, and says so out loud', () => {
    ...
    // -as-`required` divergence pointing the other way, and this package has

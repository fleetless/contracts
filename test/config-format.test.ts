...
/**
 * The validation code each refusal carries in `params`, in issue order.
 *
 * Reading the code rather than the message is the whole point of `params`:
 * the cloud maps each issue to one of thirteen codes and its repair by this
 * field — a test asserting the prose instead would pass while that join
 * broke.
 */
...
describe('parameter', () => {
  ...
  it('gives bool no constraints at all', () => {
    ...
    // **`enum: [true]` can't reach the guard**: `enum`'s element type is
    // `string | number`, so a boolean fails `invalid_union` at `enum[0]`
    // before `superRefine` runs — this test, written with `enum:[true]`,
    // stayed green with the guard deleted. `['a']` is well-typed and gets
    // there.
    //
    // `.success` alone wouldn't be enough either: guard gone, the per-entry
    // check still refuses at `enum.0`. Path and code are what pin *this*
    // rule — and they do go red when the guard is deleted (measured). The
    // float case above is the only one this guard alone can refuse.
...
describe('message template', () => {
  ...
  /**
   * `safeParse`'s contract is to return, not throw. The recursive
   * `placeholderNames` broke it: 20 000 levels raised
   * `RangeError: Maximum call stack size exceeded`, which `publisherConfig`
   * propagated — the draft PUT answered 500 where it meant 400. A
   * flow-style YAML one-liner reaches that depth in ~120 KB.
   */
  ...
  it('(break test: messageMap 200-message limit) refuses more than 200 shared messages', () => {
    ...
    // Boundary: exactly 200 must still pass.
...
describe('every refusal names its validation code', () => {
  /**
   * The mapper turns a zod issue into one of the thirteen validation codes and
...
describe('a default must satisfy the constraints it was declared beside', () => {
  ...
  // documentation calls min_value/max_value the speed limit that holds,
  ...
  // template walk without re-checking bounds, deliberately. An out-of-range
  // default published a value no caller could have sent.

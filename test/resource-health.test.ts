...
describe('the two questions one health entry used to answer', () => {
  ...
  it('requires the facet rather than defaulting it', () => {
    ...
    // the contract prefers — the conflation this field exists to end, moved
    // one layer down.
...
describe('a viewer learns why its own session ended', () => {
  ...
  it('names the session it is about', () => {
    // `liveSessionResponse` carries a session_id so a session can be
    // addressed; an event that ends one without naming it sends the console
    // back to guessing which tab lost what.
  ...
  it('distinguishes a peer release from a config change', () => {
    // These two once shipped as one message, indistinguishable to the
    // client; the fix then misreported, reading a sticky state after the
    // fact instead of carrying the reason with the ending.

...
describe('the route manifest', () => {
  ...
  it('has a one-sentence summary, and a status its handler actually answers, on every entry', () => {
    for (const r of ROUTES) {
      ...
      // this package keeps paying for. The missing-asset placeholders exist to
  ...
  /**
   * **The federated MCP callback, which the two-space redesign leaves with no flow to resume.**
...
describe('the route artifacts', () => {
   ...
   *
   * **Scoped by name suffix**, which is the convention this package
  ...
  describe('the per-schema JSON Schema artifacts', () => {
    ...
    it('every exported schema has an artifact whose bytes match a fresh render', () => {
      ...
      // nothing, which is the first failure mode in the list this package keeps.
 ...
 *
 * The cloud registers these routes later and its
 ...
 * nothing about the seventeen beside it, and this package has paid for that
 * shape seven times over: an enum guard that checked arity, an identity
...
describe('the app-user auth surface', () => {
   ...
   *
   * Login, refresh and logout came first and carry exactly this shape —
  ...
  const PUBLIC_CLIENT = [
    ...
    // The three public `GET`s added with per-app OIDC. They are members of the family
   ...
   * on no method.**
   *
   * This narrowed to `POST` because the three new `GET`s do not all
   ...
   * nor the per-route pin below, and held to nothing at all.
   *
  ...
  const TOKEN_SPENDING = [
    ...
    'POST /api/client/password/reset/confirm',
    'POST /api/client/invitations/accept',
    // Added with per-app OIDC. Not a mailed link, and the argument still lands: the one-time
   ...
   * release, but a grep across the cloud, the SDK, the console and
   ...
   * can receive — the third failure mode in the list this package keeps — on four routes
  ...
  it('answers a short password with validation_error on every route that takes one', () => {
    ...
    for (const r of rows) {
      ...
      expect(errors, `${key(r)} lists weak_password, which the cloud never emits`).not.toContain('weak_password')
  ...
  /**
   * The two `202`s that answer a body. Everything else in this release that
...
describe('app deletion', () => {
  it('offers a preview beside the delete, and the delete is Owner tier', () => {
    ...
    expect(preview?.ownerTier).toBe(false)
...
/**
 * **The per-app OIDC surface, pinned as two sets and one path.**
 *
 ...
 * through. The cloud registers exactly these, and its
 * `route-manifest.test.ts` asserts set equality with `ROUTES` in both
 * directions — so what is asserted here is what the cloud must serve.
 *
 * Written the way the app-auth block above is, for the reason stated there: a
 * guard against one hand-picked row proves that row and nothing about the
 * eight beside it.
 */
describe('the per-app OIDC surface', () => {
  ...
  it('adds exactly nine rows for the per-app OIDC surface, and no tenth path under either prefix', () => {
  ...
  /**
   * The developer half, held to the same four properties the app-auth
   ...
   * unreachable, which is the third failure mode in the list this package keeps.
   */
   ...
   *
   * The app-auth family could assert "unauthenticated ⇒ limited" over the whole
   ...
   * nothing.
   *
   * **The callback moved into the limited set** in a later fix. It was
  ...
  /** The code per-app OIDC introduces, registered rather than assumed. */
...
/**
 * **The per-app MCP surface, pinned as three sets, six paths and one
 * error code.**
 *
 ...
 *. The cloud registers exactly these, and its
 * `route-manifest.test.ts` asserts set equality with `ROUTES` in both
 * directions.
 *
 * Written the way the app-auth and per-app OIDC blocks are, for the same
 * reason: a guard against one hand-picked row proves that row and nothing
 * about the ten beside it, and every one of these was reported by a reviewer
...
describe('the per-app MCP surface', () => {
  ...
  it('adds exactly thirteen rows for the per-app MCP surface, and nothing else under any of the prefixes', () => {
   ...
   * proxy routes on.
   *
   * **The two `.well-known` paths are the reason this test exists.**
  ...
  it('opens both metadata documents without a credential and answers 404 for a disabled app', () => {
    ...
    // Non-vacuity: `mcp_disabled` IS answered somewhere in this set, so the
   ...
   * the per-app OIDC callback pin is built on, and for the same reason.
   */
   ...
   * way the per-app OIDC block reads the developer guard: `CLIENT_GUARD` is private
...
describe('the parked-items round', () => {
  ...
  it('gives every documented JSON route a response schema or a content type', () => {
    ...
    // on the file itself. The per-app endpoint is that same surface with
   ...
   * demand changes this package has argued against in writing: the
   ...
   * of `/openapi.json`. The schema is no longer strict, agreeing with

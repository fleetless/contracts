 ...
 * without ever adding it to `ERROR_CODES`.
 *
 * Worth keeping, because it will recur: a change to an *existing* shape
 * breaks the tests that already cover it, forcing a look. A **new** shape
 * breaks nothing, and additions are not self-policing.
 */
...
describe('error codes', () => {
  it('registers every code this area introduced', () => {
    // The convention this file restores. `robot_in_use` was added correctly
    // yet unasserted; `robot_deletion_partial`, added later by a different
    // hand, reached a route without ever reaching this list.
...
describe('health', () => {
  it('describes the snapshot and the push with ONE list of states', () => {
    // Two literal enums linked by nothing, agreeing only because whoever
    // added `unknown` remembered both places — the artifacts published two
    // independent copies, no `$ref`.

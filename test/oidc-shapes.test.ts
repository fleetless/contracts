...
/**
 * **What is left of the federation shapes after the two-space cut** (redesign of
...
describe('idpIssuer — the string that decides where the server connects', () => {
   ...
   * the server's own disk. This checks that a scheme allow-list, not a
   * `z.url()`, stands here — the defect found by storing
   ...
   * mistake or an attempt to redirect it.
   */
...
describe('the group-scoped federation shapes are gone', () => {
  ...
  const DELETED = [
    ...
    // `GET /mcp/oauth/idp-callback`, a route the redesign leaves unreachable and this
  ...
  it('and the replacement is exported, so this file is not asserting an empty world', () => {
    ...
    // The per-app callback vocabulary — what a reader looking for the

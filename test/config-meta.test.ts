 ...
 *
 * A configuration editor renders `description` from the exported JSON Schema
 * — skip it and a developer hovers and learns nothing. This test is the
 * guarantee: a field added later without one fails here, not as a blank
 * tooltip in production.
 *
 * `NODES` grows one entry per task — every property under a listed node
 * must be described.
 */
 ...
 * `{ type, propertyNames, additionalProperties }` — **no `properties`**.
 * The value schema is what's interesting, so stepping into `datapoints`
 * must land on the datapoint, not the map around it. As of 2026-09-02,
 * `z.toJSONSchema(robotConfigDoc)` inlines everything — no `$defs`, no
 * `$ref` — so a plain walk suffices.
 */
 ...
 * …). As of 2026-09-02 the export inlines everything, no `$ref`, so the
...
describe('every documented field carries a hover text', () => {
   ...
   * because the document had yet to be described. Unskipped in
 ...
 * empty and pass vacuously — the `.every()`-over-nothing failure this package
 * keeps finding.
 *

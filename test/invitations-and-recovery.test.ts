 ...
 *
 * **Reduced by the 2026-08-29 identity redesign.** The two invitation
...
describe('the limit', () => {
  it('tells the caller when to come back, and nothing about the defence', () => {
    // "Too many" without "in 800 ms" invites an immediate retry — the
    // behaviour the limit exists to stop. The omission would make the
    // refusal part of the attack.
 ...
 * belongs to an app, not an org` were deleted here on 2026-08-29, not
 * quietly dropped.**
 *
 * The first pinned that the developer and end-user invitation shapes rejected
 * each other's payloads — a credential from one identity space must never
 * authenticate the other. There is one space now, and one
 * `createUserInviteRequest`; the property it guarded can no longer be stated,
 * and keeping the test would assert a distinction the platform no longer
 * makes.
 *
 * The second pinned `selfRegistration`'s four controls — the empty-domain-list
 * reading in particular. That policy dies with no successor; its reasoning
 * lives on, in words, at the deletion tombstone in `src/identity.ts` — JIT
 * provisioning inherits every one of its rules.
 */

describe('tiers', () => {
  it('names the tier required and the one held, and nothing about the target', () => {
    // The rule survives the owner/member -> owner/developer rename. It says
    // nothing about whether the target exists — only what the caller could
    // already read off the documentation.

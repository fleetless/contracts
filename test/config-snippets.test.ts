 ...
 * what goes there, and the editor used to answer with words scraped
 * out of their own document.
 */
const SECTIONS = ['messages', 'datapoints', 'actions', 'services', 'publishers', 'cameras']

/**
 * A snippet body with its syntax undone, so the values it was authored from
 * can be judged against the format.
 *
 ...
 *   the editor inserts for a developer who tabs past it. Checked against
 ...
 *   and **deletes** — checked against the same `SnippetParser`: `x: ${speed}`
...
describe('every section offers a whole entry', () => {
  ...
  /**
   * The snippet must be writable as it stands: a body that fails validation
   * inserts a document the format refuses — worse than offering nothing, since
   * the developer trusts it.
   *
   * `fill()` above undoes the snippet syntax first; what it handles, and why
   * each part is load-bearing, is documented there.
   *
   ...
   * three is the round trip in the console's own editor suite,
...
/**
 * The four camera sources. `source:` is where the complaint started: a
...
describe('every nested object offers a skeleton', () => {
   ...
   * that is what has to agree. A guard shaped like your own code,
 ...
 * `front: ▮` under `cameras:`.
 *
 * The 135-position sweep that started this work emitted a value probe for each
 * of a node's **keys**, and a map entry is not a key, so it never asked this
 * question anywhere. Run in a browser on 2026-09-03, after three of the fixes
 * had already shipped: eight positions, every one silent — the same
...
describe('every map entry offers a whole entry', () => {
   ...
   * constant. Three reviews have caught the same drift: a second
   ...
   * clone's metadata with its parent's** rather than replacing it. Checked
 ...
 * what somebody remembered. That has already been paid for once here: the
 ...
 *   calls the headline fix uncovered.
 ...
 * the fifth, `messages.<slug>`, gained one later. They are the same zod
...
describe('every value position a skeleton belongs at carries one', () => {
   ...
   * chosen here, so it counts as covered when **every** branch carries one.
   ...
   * buckets. The third exists so nothing can fall out of the walk
  ...
  /**
   * The positions the sweep measured, written out rather than counted, so the
   * number below falls out of a list somebody can read and disagree with.
   *
   ...
   */
  it('the walk reaches exactly the positions the sweep measured', () => {
    const { mappings } = walk()
  ...
  it('the schema-free positions are exactly the four excluded, plus the shared message', () => {
    ...
    // Reachability first, so a mistyped or retired path says so in its own
    // words rather than as one line of a set difference.

...
// zod 4's z.uuid() enforces version/variant nibbles, so
...
describe('alertCondition — discriminated union on kind', () => {
  it('above requires a numeric threshold', () => {
    ...
    // Required-field pin: `above` with no threshold must be rejected, not
    // silently accepted as "no bound".
...
describe('datapointAlert — the entity, definition + runtime state together', () => {
  ...
  // No mail path anymore. This shape is a plain `z.object` and strips unknown
  // keys rather than refusing them, so the pin checks the parsed output: old
  // fields a caller still sends get silently dropped, never surfaced as a
  // mail setting that means nothing.
...
describe('slugUsageResponse — alert_count: the rename dialog counts alerts too', () => {
  const USAGE = {
    grant_count: 2,

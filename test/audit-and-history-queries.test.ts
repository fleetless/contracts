...
describe('the audit log can be paged and filtered', () => {
  ...
  it('refuses a cursor JavaScript cannot represent, rather than handing Postgres an out-of-range bigint', () => {
    // The regex admits 19 digits, which `Number()` renders as 1e19 — an
    // integer to `Number.isInteger`. `.int()` bounds the SAFE-integer range
    // instead, not obvious from `.int().positive()`; loosening it silently 500s.

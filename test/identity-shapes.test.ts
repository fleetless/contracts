...
/**
 * **The Fleetless-user half of the two identity spaces**, as redesigned on
 * 2026-09-05: the org's team, its invitations and its
...
describe('fleetlessUser — the team, and the wire says nothing about the credential', () => {
   ...
   * to the one shape a console renders on every page: carrying the hash puts
   * it in every log that ever captured a response.
   */
...
describe('patchFleetlessUserRequest — email is immutable, and a tier change is not a patch', () => {
   ...
   * failure mode this package has already paid for once, where *field sent* and
...
describe('the group-and-assignment model is gone from the contract', () => {
  /**
   * A greenfield cut: the deleted shapes must not come back as aliases — that
   * is how a deleted model quietly survives. This list earned its keep once
   * already: it missed one shape for a round, and a reviewer re-added both the
   * shape and its barrel export with the whole suite green.
   */
  const DELETED = [

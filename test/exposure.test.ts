...
describe('exposure model', () => {
  ...
  it('indexes at most one array level per segment, because ROS has no nested arrays', () => {
    // ROS 2 IDL has float64[], float64[3], float64[<=10] — never float64[][].
    // A second index denotes nothing; the bridge already raises FieldPathError
    // for it, so the grammar must agree.
...
describe('REST shapes', () => {
  ...
  it('refuses a draft response with no source — a reader always has text to show', () => {
    // `source` is not optional on purpose: a draft exists before anyone has
    // typed anything, and for that one the server renders the document rather
    // than sending nothing. Optional would push that decision onto every
    // reader.
    expect(

 ...
 *
 * Given a git tag (CI_COMMIT_TAG or argv[2]) it fails unless package.json's
 * version equals the tag without its leading `v`, and prints the npm dist-tag
 * the publish job must use: `latest` for X.Y.Z, `next` for a pre-release
 * (X.Y.Z-beta.1 and the like). Output is one line, `DIST_TAG=<latest|next>`,
 * so a shell can `eval` it.
 *

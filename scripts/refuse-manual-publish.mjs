...
/**
 * `npm publish` from a working tree is refused. CONTRIBUTING says "the pipeline
 * publishes; nobody runs `npm publish` by hand" — this is the mechanism behind
 * that sentence, which was previously a rule with nothing enforcing it.
 *
 ...
 *
 * **It does not stand in the pipeline's way, and the reason is a detail of
 ...
 * of the way: `npm publish <tarball>`, which is what the pipeline does.
 */
console.error(`
  Refusing to publish @fleetless/contracts from a working tree.

  Releases are made by the GitLab pipeline, from the tarball its \`verify\` job
  packed after the typecheck, the suite, the artifact staleness gate and
  \`pnpm run test:pack\`. See "Releasing" in CONTRIBUTING.md.

    1. bump the version in package.json and add a dated CHANGELOG.md heading
    2. commit, push, let \`verify\` go green
    3. tag vX.Y.Z and push the tag

  If you are the pipeline and you are seeing this, you are publishing a

 ...
 *
 * The change under test is additive by claim; this script turns the claim into a
 ...
 * point it stays cumulative for the whole branch and cannot be made trivially
...
/** The commit the branch started from. Every comparison is against it. */
 ...
 * blind spot sits exactly on the surface the change hangs `.meta()` on.
 *
...
for (const file of files) {
  ...
  try {
    ...
    console.error(`NEW FILE: ${file} — this branch adds no schema, so a new artifact is a mistake`)
    failures += 1
    continue
  }
